...
const nginxParser: Parser = {
  astFormat: "nginx",
  parse: (text: string, options: NginxOptions): ASTNode => {
    const parseRecursive = (t: string, rootIndex: number): ASTNode[] => {
      ...
      const breakToken = () => {
        ...
        const end: number = tokenBuffer[tokenBuffer.length - 1][0] + rootIndex;
...
const plugin: Plugin = {
  ...
  parsers: { nginx: nginxParser },
  printers: { nginx: nginxPrinter },
  options: {
    alignDirectives: {
      type: "boolean",
      category: "nginx",
